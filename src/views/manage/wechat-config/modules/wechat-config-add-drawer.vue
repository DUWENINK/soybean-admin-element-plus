...
import { fetchAddWechatAppConfig } from '@/service/api';
import { useForm, useFormRules } from '@/hooks/common/form';
import { $t } from '@/locales';
import SystemEnumSelect from '@/components/custom/system-enum-select.vue';

defineOptions({ name: 'WechatAppConfigAddDrawer' });
...
type AddModel = Omit<Api.SystemManage.WechatAppConfig, keyof Api.Common.AuditRecord | keyof Api.Common.BaseRecord>;
...
async function handleSubmit() {
  ...
  const data = await fetchAddWechatAppConfig(model.value);
...
<template>
  <ElDrawer v-model="visible" :title="$t('page.manage.wechatConfig.add')" :size="360">
